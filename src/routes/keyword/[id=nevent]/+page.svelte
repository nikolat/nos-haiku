<script lang="ts">
	...
	const getEventPointer = (urlId: string | undefined): nip19.EventPointer | undefined => {
		if (urlId === undefined) {
	...
	afterNavigate(() => {
		currentChannelPointer = getEventPointer(page.params.id);
	});
</script>

<App up={{ currentChannelPointer }} />
